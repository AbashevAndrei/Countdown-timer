...
let appointedTime = new Date(1977, 0, 15)
// Текущая дата
let today = new Date()
...
function timeFromEvent() {
  // document.createElement('div.selector')
  let appointedTime = new Date(1977, 0, 15)
  let now = Date.now()
  let counterFrom = document.querySelector('.counterFrom')
  // Находим разницу между датами в миллисекундах
  let timeDifferenceInMilliseconds = now - appointedTime
  ...
  counterFrom.textContent = `Со дня рождения прошло: ${displayYears} л. ${displayDays} дн. ${displayHours} : ${displayMinutes} : ${displaySeconds}`
